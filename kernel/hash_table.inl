...
// reserves space
inline void UidHashTable::reserve(size_t size)
{
	reserve_size = size;
	_reserve();
}
inline void UidHashTable::_reserve()
{
	size_t c = reserve_size / DB_UID_SPLIT;
	for (size_t i = 0; i < DB_UID_SPLIT; ++i)
		db_uid[i].reserve(c);
}
...
inline const std::vector<UidType>& UidHashTable::get_uid_coeffs() const
{
    return uid_coeffs; 
}
